#!/bin/bash
# Exercises machines/qi (the PowerPC emulator/debugger) a bit beyond
# just loading a header (see tests/libmach/smoke.sh for that): symbol
# resolution + disassembly, and actually running a real Plan9 PowerPC
...
# target: qi links against lib_toolchain/libmach, so a wrong
# disassembly/symbol/register-access result here would mean access.c/
# sym.c/machdata.c/qdb.c regressed).

set -e

QI=${QI:-qi}
EXE=${EXE:-"$(dirname "$0")/../s/mini/hello_plan9_power.exe"}
...
# resolves it to _main (exercises sym.c + qdb.c's disassembler
# together). ---
out=$(printf '_main?i\n$q\n' | timeout 5 "$QI" "$EXE" 2>&1)
if echo "$out" | grep -q "^_main?" && echo "$out" | grep -qi "SUB"; then
    ok "qi: disassembles entry instruction with correct symbol (_main)"
else
    bad "qi: entry disassembly/symbol resolution wrong (got: $out)"
fi
...
if echo "$out" | grep -q "Hello, world" && echo "$out" | grep -q "exits(0)"; then
    ok "qi: runs real Plan9/PowerPC executable to completion (exit syscall reached)"
else
    bad "qi: failed to run to completion (got: $out)"
fi
