...
# fix float/64bits ops SEMI arm32 and arm64 tests/c/mini2,
...
*** LATER remove dead code like maxbecome/frame in linkers that was used for Alef
tail-call opti
...
**** less: run mk objtype=arm test_one in c/mini2/ too, remove NOSPLIT check
by using ifdef
...
**** less: tests/c/mini2/ does not work on arm64 even with basic helloprintf.c
that calls test()

if you use strace you see it's completely wrong! calling 
 write(1, 0x5, 4195588) ????
weird because tests/c/mini/ works fine with arm64 and looks similar

**** goken: get tests/c/mini2/ working for arm64, DWORD error
...
DWORD $buf-100(SP)
  because of byte buf[100];  and some operation around
  it

try make minimal test case and put in tests/c/mini/helloc.c instead
...
**** less: run mk objtype=mips test_one in c/mini2/
