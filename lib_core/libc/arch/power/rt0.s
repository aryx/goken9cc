...
//
// The ADD $-8,R1,R1 below is a second, separate adjustment (same
// shape as alpha's own SUBQ): main(argc, argv)'s own outgoing call
// needs a real 4-byte home slot for argv (its stack-passed second
// argument) at 4(R1) after THIS adjustment -- confirmed via a qc -S
// probe on an equivalent 2-arg call, the same "count+4(FP)" packing
// every other multi-arg stub in this tree already uses.
TEXT _main(SB), $0
	MOVW	$setSB(SB), R2
	MOVW	8(R1), R3		// argc
	...
	ADD	$-8, R1, R1
	MOVW	R4, 4(R1)		// outgoing argv slot for main(argc, argv)
	BL	main(SB)
	MOVW	$0, R3
	BL	exit(SB)
loop:
