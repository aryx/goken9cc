...
/* dirfstat()/dirfwstat() (include/os/stat.h) for linux/sparc. See
 * stat_amd64.c's header comment for the overall design.
 *
 * Struct layout is the kernel's own 32-bit sparc `struct stat64`
 * (what SYS_fstat64 fills) -- confirmed against this host's installed
 * sparc64-linux-gnu cross headers, /usr/sparc64-linux-gnu/include/
 * asm/stat.h's own "32 bit sparc" #else branch, the same reference
 * already used for numbers_sparc.h's own syscall numbers.
 * Unlike stat_386.c, this arch's stat64 has no separate 32-bit legacy
 * inode field to skip past -- st_ino is already the real 64-bit
 * inode, used directly for qid.path below. Every field here is a
 * fixed-width kernel type (unsigned long long/unsigned int), so this
 * project's own 4-byte `long` never collides with anything, same as
 * every other 32-bit arch's own stat_$cputype.c.
 */
