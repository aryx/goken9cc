/* Companion header for the (empty) decl-generated zsyscall_plan9_power.c
 * -- see syscall_plan9_power.decl's own comment for why this arch needs
 * no shared trampoline. read()/write()/exit() are hand-written here
 * (not raw syscalls themselves, so they don't belong in svc_power.s)
 * instead of decl-generated, since none of them is a plain 1:1 syscall
 * wrapper mksyscall.sh's model supports:
 *
 * - read()/write() wrap the raw pread()/pwrite() syscalls (svc_power.s)
