/* Companion header for the (empty) decl-generated zsyscall_plan9_sparc.c
 * -- see syscall_plan9_sparc.decl's own comment for why this arch needs
 * no shared trampoline. read()/write()/exit() are hand-written here
 * (not raw syscalls themselves, so they don't belong in svc_sparc.s)
 * instead of decl-generated, since none of them is a plain 1:1 syscall
 * wrapper mksyscall.sh's model supports:
 *
 * - read()/write() wrap the raw pread()/pwrite() syscalls (svc_sparc.s)
